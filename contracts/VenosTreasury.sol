...
contract VenosTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SVNO }

    address public immutable VNO;
    ...
    address public sVNO;
    uint public sVNOQueue; // Delays change to sVNO address
    ...
    /**
        @notice allow approved address to deposit an asset for VNO
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint VNO needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( VNO ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn VNO for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IVNOERC20( VNO ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sVNO ).balanceOf( msg.sender ); // Can only borrow against sVNO held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with VNO
        @param _amount uint
     */
    function repayDebtWithVNO( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IVNOERC20( VNO ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, VNO, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( VNO ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( VNO ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns VNO valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match VNO decimals
            value_ = _amount.mul( 10 ** IERC20( VNO ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SVNO ) { // 9
            sVNOQueue = block.number.add( blocksNeededForQueue );
        } else return false;
