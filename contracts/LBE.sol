...
contract GRDDV2Sale {
    ...
    constructor(address _baseToken, address _farmingToken, address _owner, address _base) {
    ...
    function invest() public onlyEOA {
        require(startStatus, "!start");
        require(!investor[msg.sender], "invested");
        require(IERC20(farmingToken).balanceOf(address(this)) > 1000 * 10 ** 9, "sold out");     
        ...
        if(count > 0 && count % 2 == 0) {
            IERC20(baseToken).transfer(base,b);
            IERC20(farmingToken).mint(msg.sender, _amount / (5 * 10 ** 9));
        }   
        else {
            IERC20(baseToken).transfer(owner,b);
    ...
    function getInvestStatus(address _account) view external returns(bool) {
